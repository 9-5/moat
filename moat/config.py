...
def load_config(force_reload: bool = False) -> MoatSettings:
    ...
    with open(CONFIG_FILE_PATH, 'r') as f:
        ...
        if config_data is None:
            config_data = {}  # Treat empty file as empty dict, avoid errors
...
def get_settings() -> MoatSettings:
    if _settings is None:
        raise ValueError("Settings not initialized. Ensure config.yml exists and is valid.")
    return _settings

async def save_settings(new_settings: MoatSettings) -> bool:
    """Validates the new settings, then saves to file if valid."""
    global _settings, _config_last_modified_time
    
    try:
        validated_settings = new_settings.model_copy(deep=True)  # Create a copy to avoid modifying the original
        
        # Save to YAML file
        with open(CONFIG_FILE_PATH, 'w') as f:
            yaml.dump(validated_settings.model_dump(), f, sort_keys=False) # Changed to model_dump()
