...
async def update_config(
    ...
    try:
        ...
        # Attempt to save the settings, which also validates them
        if save_settings(new_config_dict):
            # Apply the changes to the runtime configuration
            new_settings = load_config(force_reload=True)
            await apply_settings_changes_to_runtime(get_settings(), new_settings)
